...
PHP_FUNCTION(IupSetRGBA)
{
    ...
    if (zend_parse_parameters(argc TSRMLS_DC,"rsllll",&ihandle_res,&name,&name_len,&r,&g,&b,&a) == FAILURE) {
        return;
    }
...
/* {{{ proto resource IupGetRGBA(resource ih, string name, ref r, ref g, ref b, ref a)
   ;
 */
PHP_FUNCTION(IupGetRGBA)
{
    ...
    if (zend_parse_parameters(argc TSRMLS_DC,"rszzzz",&ihandle_res,&name,&name_len,&rr,&gg,&bb,&aa) == FAILURE) {
        return;
    }

    ih = zend_fetch_resource_ex(ihandle_res,"iup-handle",le_iup_ihandle);

    IupGetRGB(ih,name,&r,&g,&b,&a);
